 ...
 *
 */
#ifndef __RESTRUCTOR_RESTRUCTOR_CPU_H_
#define __RESTRUCTOR_RESTRUCTOR_CPU_H_
...
/** @brief CPU加速重建器 */
class RestructorCpu : public Restructor {
  public:
    /**
     * @brief 构造函数
     *
     ...
     */
    RestructorCpu(IN const tool::Info &calibrationInfo);
    /**
     * @brief 重建
     *
  ...
  protected:
     ...
     * @param depthImgOut 深度图(CV_32FC1)
     */
    void getDepthColorMap(IN const cv::Mat &leftAbsImg,
                          IN const cv::Mat &rightAbsImg,
                          IN const RestructParamater param,
  ...
  private:
     ...
     * @brief 获取深度纹理入口函数
     *
     * @param leftAbsImg 左相机绝对相位(CV_32FC1)
     * @param righAbstImg 右相机绝对相位(CV_32FC1)
     * @param param 重建器重建参数
     * @param depthImgOut 深度图(CV_32FC1)
     * @param region 执行区域（行）
     */
    void entryDepthColorMap(const cv::Mat &leftAbsImg,
...
#endif // !__RESTRUCTOR_RESTRUCTOR_CPU_H_
