...
void remapToDepthCamera(const cv::Mat &depthIn, const cv::Mat &textureIn,
    ...
    std::mutex mutexMap;
    const int chnelStep = (depthIn.type() == CV_32FC1) ? 1 : 2;
        ...
        threads[i] = std::thread([&, i] {
            for (int rowIndex = rowPerThread * i;
                ...
                for (int colIndex = 0; colIndex < depthIn.cols; ++colIndex) {
                    const int elementLoc = colIndex * chnelStep;
                    if (!ptrDepthIn[elementLoc]) {
                        continue;
                    }
                    
                    Eigen::Vector3d pointBefore;
                    const float subpixelLocX = chnelStep == 1 ? colIndex : ptrDepthIn[elementLoc + 1];
                    pointBefore << ptrDepthIn[elementLoc] * (subpixelLocX - cx) / f,
                        ptrDepthIn[elementLoc] * (rowIndex - cy) / f,
                        ptrDepthIn[elementLoc];
