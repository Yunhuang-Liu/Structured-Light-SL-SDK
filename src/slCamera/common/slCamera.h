...
/** @brief 单帧数据 */
struct FrameData {
    cv::Mat __depthMap;   //深度图
    cv::Mat __textureMap; //纹理图
    Cloud __pointCloud;   //点云
};
/** @brief 相机信息 */
struct SLCameraInfo {
    std::string __cameraName; //相机名
    cv::Mat __intrinsic;      //内参
    bool __isFind;            //是否查找到该相机
};
/** @brief 结构光相机 */
class SLCamera {
  public:
     ...
     * @return false 失败
     */
    virtual bool offLineCapture(IN const std::vector<cv::Mat>& leftImgs, IN const std::vector<cv::Mat>& rightImgs, OUT FrameData &frameData) = 0;
    /**
     * @brief 是否使能深度相机(@note
