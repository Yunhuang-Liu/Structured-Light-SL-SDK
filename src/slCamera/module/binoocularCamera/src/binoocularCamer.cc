...
bool BinocularCamera::capture(FrameData &frameData) {
    ...
    const int imgSizeWaitFor =
        (__numbericalProperties["Restruction Method"] == 0 ? __numbericalProperties["Phase Shift Times"] +
                   __numbericalProperties["Gray Code Bits"]
             : __numbericalProperties["Gray Code Bits"] +
    ...
    if (__booleanProperties["Enable Depth Camera"]) {
        ...
        std::vector<cv::Mat> leftImgsOrigin, rightImgsOrigin;
        ...
        for (size_t i = 0; i < leftImgs.size(); ++i) {
            ...
            rightImgsOrigin.emplace_back(rightImg);
